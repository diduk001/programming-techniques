#!/usr/bin/env python3
"""Generates report.pdf: cover page (two plots + repo link), README.md, then docs.pdf."""
...
REPO_URL = "https://github.com/diduk001/programming-techniques/tree/main/practice2"
BASE = pathlib.Path(__file__).parent.parent
RESULTS = pathlib.Path(__file__).parent / "results"
PLOT_SEARCH = RESULTS / "search_performance.png"
PLOT_COLLISIONS = RESULTS / "hash_collisions.png"
DOCS = BASE / "docs.pdf"
...
def build_cover() -> bytes:
    ...
    img_w = page_w
    img_h = img_w * 0.6  # fits two plots on one page

    story = [
        Paragraph("Лабораторная работа 2", title_style),
        Spacer(1, 0.4 * cm),
        Image(str(PLOT_SEARCH), width=img_w, height=img_h),
        Spacer(1, 0.3 * cm),
        Image(str(PLOT_COLLISIONS), width=img_w, height=img_h),
        Spacer(1, 0.4 * cm),
...
def main():
    for path in (PLOT_SEARCH, PLOT_COLLISIONS, README, DOCS):
        if not path.exists():
